...
heartbeatConfigTests = testGroup "HeartbeatConfig"
  [ testCase "has sensible defaults" $ do
      let defaultHb = HeartbeatConfig
            { hbEnabled = True
...
cronJobTests = testGroup "CronJob"
...
compactionConfigTests = testGroup "CompactionConfig"
  [ testCase "has sensible defaults" $ do
